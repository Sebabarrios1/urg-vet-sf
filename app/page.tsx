...
import {
  ...
  Phone, MapPin, Menu, MessageCircle,
...
const PETS = [
  { name: "Caninos", img: "https://images.unsplash.com/photo-1543466835-00a7907e9de1?auto=format&fit=crop&w=800&q=80" },
  { name: "Felinos", img: "https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba?auto=format&fit=crop&w=800&q=80" }
];
...
const SPECIALTIES = [
  { title: "ECOGRAFÍA", desc: "Diagnóstico por imágenes mediante ecografías.", icon: <Monitor size={28} /> },
  { title: "DERMATOLOGÍA", desc: "Diagnóstico y tratamiento de enfermedades de la piel.", icon: <Stethoscope size={28} /> },
  { title: "CARDIOLOGÍA", desc: "Evaluación cardiológica y estudios especializados.", icon: <Activity size={28} /> }
];
...
const TEAM = [
  {
    ...
    icon: <Sun size={14} />,
    color: "bg-[#2B5289]",
    img: "https://images.unsplash.com/photo-1594824436998-058b23c9121b?auto=format&fit=crop&w=300&q=80",
    facePos: "center 15%"
  },
  {
  ...
  },
  {
    ...
    color: "bg-[#1a3a5c]",
    img: "https://images.unsplash.com/photo-1612349317150-e410f624c427?auto=format&fit=crop&w=300&q=80",
    facePos: "center 5%"
  },
  {
    name: "Luis Prieto",
...
const FAQS = [
  ...
  { q: "¿Se coordinan derivaciones?", a: "Sí, coordinamos derivaciones con especialistas como Tamara Silvetti, Nicolás Picciochi y Valeria Gálvez para interconsultas específicas gestionadas desde la clínica." }
];
  ...
  return (
    <div className="font-sans min-h-screen bg-white" style={{ fontFamily: "'League Spartan', sans-serif" }}>
        ...
        <div className="max-w-6xl mx-auto px-6 h-[70px] flex items-center justify-between">
          <a href="#" className="flex items-center gap-3 text-white no-underline">
            <div className="bg-white rounded-xl p-1 shrink-0 flex items-center justify-center w-[48px] h-[48px] overflow-hidden">
              <div className="w-full h-full bg-gray-200 text-[#2B5289] flex items-center justify-center font-bold text-xs">LOGO</div>
            </div>
      ...
      <section className="w-full bg-[#2B5289] relative overflow-hidden" style={{
        backgroundImage: `linear-gradient(90deg, rgba(43, 82, 137, 0.95) 0%, rgba(43, 82, 137, 0.85) 45%, rgba(43, 82, 137, 0.3) 100%), url('https://images.unsplash.com/photo-1583337130417-3346a1be7dee?auto=format&fit=crop&w=1920&q=80')`,
        ...
        <div className="max-w-6xl mx-auto px-6 py-16 md:py-24 flex flex-col md:flex-row relative z-10 items-center">
          ...
          <div className="hidden md:flex flex-col items-center bg-white/10 border border-white/20 rounded-2xl p-8 backdrop-blur-md ml-10">
            ...
            <div className="text-white text-sm font-bold tracking-widest uppercase">Atención</div>
            <div className="text-white text-5xl font-black">24 HORAS</div>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {SERVICES.map((s, idx) => (
              <div key={idx} className={`rounded-2xl p-8 transition-all hover:shadow-xl ${s.urgent ? 'bg-[#DB1E26] text-white' : 'bg-[#2B5289] text-white'}`}>
                ...
                <h3 className="text-[26px] font-black mb-3 leading-tight uppercase">{s.title}</h3>
                <p className="text-[17px] opacity-90 leading-relaxed font-medium">{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="mb-8">
            ...
            <p className="text-gray-600 font-semibold text-lg mt-2">
              Atención con profesionales especializados y turnos programados. Servicios brindados por profesionales externos, gestionados y coordinados directamente por nuestra clínica.
            </p>
          </div>
            ...
            {SPECIALTIES.map((spec, i) => (
              <div key={i} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 font-medium">{spec.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="max-w-5xl mx-auto px-6">
          <div className="bg-[#fff0f1] rounded-3xl p-8 md:p-12 border-2 border-[#DB1E26]/20 shadow-sm">
            <h2 className="text-3xl md:text-4xl font-black text-[#DB1E26] mb-6 flex items-center gap-3">
              <AlertCircle size={36} /> ¿Qué es una urgencia?
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
              {URGENCY_SIGNS.map((sign, i) => (
                <div key={i} className="flex items-start gap-3 text-base font-semibold text-gray-700">
                  <Zap size={20} className="text-[#DB1E26] shrink-0 mt-0.5" /> {sign}
                </div>
              ))}
            </div>
              ...
              <div key={idx} className="bg-gray-50 rounded-2xl overflow-hidden shadow-sm flex border border-gray-200">
                ...
                <div className="p-5 flex-1 flex flex-col justify-center">
                  <span className={`inline-block self-start text-[10px] font-black px-2 py-1 rounded uppercase tracking-wider mb-2 ${member.type === 'clinic' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700'}`}>
                    {member.type === 'clinic' ? 'CLÍNICA GENERAL' : 'INTERNACIÓN / EMERGENCIAS'}
                  </span>
          ...
          <div className="space-y-6">
            {FAQS.map((f, i) => (
              <div key={i} className="border-b border-gray-200 pb-6">
                <h4 className="font-black text-gray-800 mb-2 text-lg flex gap-3">
                  <div className="w-2 h-2 bg-[#DB1E26] rounded-full mt-2.5 shrink-0"></div> {f.q}
                </h4>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div className="space-y-8 flex flex-col justify-center">
              <a href={`tel:${PHONE_NUMBER}`} className="flex items-center gap-5 group no-underline">
                <div className="w-14 h-14 rounded-full bg-[#DB1E26] text-white flex items-center justify-center shrink-0 shadow-lg shadow-red-200 transition-transform group-hover:scale-110">
                  <Phone size={24} />
                </div>
                <div>
              ...
              <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" rel="noreferrer" className="flex items-center gap-5 group no-underline">
                <div className="w-14 h-14 rounded-full bg-white border border-gray-300 text-[#25D366] flex items-center justify-center shrink-0 shadow-sm transition-transform group-hover:scale-110">
              ...
              <div className="flex items-center gap-5">
                <div className="w-14 h-14 rounded-full bg-[#2B5289] text-white flex items-center justify-center shrink-0">
                  <MapPin size={24} />
                </div>
                <div>
